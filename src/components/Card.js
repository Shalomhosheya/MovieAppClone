...
const Card = ({ data,trending,index ,media_type}) => {
  ...
  return (
    <Link to={"/"+media_type+"/"+data.id} className='w-full h-full min-w-[250px] max-w-[250px] h-80  block overflow-hidden rounded relative hover:scale-105 transition-transform duration-300'>
      {/* {
        data?.poster_path?(
          <img src={imageURL + data?.poster_path}/>
        ): (
          <div className='bg-neutral-800 w-full h-full flex items-center item-center'>
            No image
          </div>
        )
